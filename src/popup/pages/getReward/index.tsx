  ...
  const msgGetRewardAction = useRequest(getReward, {
    ...
    onSuccess: (res: any) => {
      if (res.code === 0) {
        ...
        return navigator("/transactionDetail", {
          state: { hash: res.transactionHash },
        });
      } else {
      ...
      <div className={styles.item}>
            ...
            {isKyc
              ? `${
                  delegation.amount == "0"
                    ? `0`
                    : `${
                        formatCountByDenom("umec", delegation.amount).amount
                      } MEC`
                }`
              : `${
                  delegation.unKycAmount == "0"
                    ? `0`
                    : `${
                        formatCountByDenom("umec", delegation.unKycAmount)
                          .amount
                      } MEC`
                }`}
          </p>
        ...
        <div className={styles.row}>
          ...
          <p className={styles.value}>
            {reward.amount == 0
              ? "0"
              : `${formatCountByDenom("umec", reward.amount).amount} MEC`}
          </p>
        </div>
        <div className={styles.row}>
        ...
        <Button
          loading={msgGetRewardAction.loading || loading}
